...
class CommandTests(SimpleTestCase):
    ...
    def test_wait_for_db_delay(self, patched_sleep, patched_check):
        ...
        patched_check.side_effect = (
            [PsycopgError] * 3 + [OperationalError] * 3 + [True]
        )
...
class ExportFeedbackCSVTests(TestCase):
    ...
    def setUp(self):
        ...
        self.survey_response = SatisfactionSurveyResponse.objects.create(
            user=self.user, version="1.0", survey=self.survey_data
        )
    ...
    def test_export_feedback_csv_with_summary(self):
        ...
        try:
            call_command(
                "export_feedback_csv", outfile=temp_path, summary=True
            )
    ...
    def test_export_feedback_csv_file_write_error(self, mock_file):
        ...
        with self.assertRaises(IOError):
            call_command(
                "export_feedback_csv", outfile="/invalid/path/file.csv"
            )
